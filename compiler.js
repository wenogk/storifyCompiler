  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith(sep)) {
      ...
      while (!lines[i].startsWith(sep)) {
        ...
        if ((i + 1 < lines.length) && (!lines[i + 1].startsWith(sep))) {
          i = i + 1;
        } else {
          break;
        }
...
function loadPath(idVal) {
  ...
  if (item.video != null) {
    let code = "<div class='embed-responsive embed-responsive-16by9' style='width:100%'><iframe src='https://www.youtube.com/embed/" + YouTubeGetID(item.video.youtubeID) + "?controls=0&autoplay=1' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen class='embed-responsive-item'></iframe></div><br/>"
    if (item.text != null) {
      code += item.text
    }
    $("#bookText").html(code);
  ...
  if (item.question == null) {
    ...
    item.question.options.forEach((option) => {
        $("#bookQuestion").append("<button class='btn btn-primary btn-lg btn-block' onclick='loadPath(" + option.id + ")''>" + option.value + "</button><br/><br/>");
    });
    prevPath = idVal;
  }
...
function helper() {
  ...
  for (var i = 0; i < lines.length; i++) {
    if (lines[i].startsWith(sep)) {
      if ((i != 0) && (optionStack[sepCounter] != null)) {
        lines[i] = sep + counter + " (" + optionStack[sepCounter] + " )";
        sepCounter += 1;
      } else {
        lines[i] = sep + counter;
      }
